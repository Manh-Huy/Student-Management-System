...
{
    public partial class AuthenticationContactForm : Form
    {
        public AuthenticationContactForm()
        {
            InitializeComponent();
        }
        USER user = new USER();
        ...
        private void AuthenticationContactForm_Load(object sender, EventArgs e)
        {
            fillGrid(new SqlCommand("SELECT id AS [ID], username AS [User Name], password AS [Password], email AS [Email] FROM Temp_Log_in_Contact"));
        }
        // Nạp data
        ...
        private void ButtonApprove_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(stringcon))
            {
                ...
                foreach (string userId in selectedIds)
                {
                    ...
                    SqlCommand selectCommand = new SqlCommand("SELECT * FROM Temp_Log_in_Contact WHERE id = @id", connection);
                    ...
                    {
                        // Thêm thông tin user vào bảng chính
                        SqlCommand insertCommand = new SqlCommand("INSERT INTO Log_in_Contact (Id, username, password, email) VALUES (@Id, @username, @password, @email)", connection);
                    ...
                    SqlCommand deleteCommand = new SqlCommand("DELETE FROM Temp_Log_in_Contact WHERE id = @id", connection);
            ...
            SqlCommand command = new SqlCommand("SELECT id AS [ID], username AS [User Name], password AS [Password], email AS [Email] FROM Temp_Log_in_Contact");
            dataGridViewTemp_UserList.ReadOnly = true;
            dataGridViewTemp_UserList.DataSource = user.getUsers(command);
        }
        ...
        private void ButtonReject_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridViewTemp_UserList.SelectedRows)
            {
                ...
                using (SqlConnection connection = new SqlConnection(stringcon))
                {
                    ...
                    SqlCommand sqlCommand = new SqlCommand("DELETE FROM Temp_Log_in_Contact WHERE id = @id", connection);
        ...
        private void ButtonApproveAll_Click(object sender, EventArgs e)
        {
            ...
            db.openConnection();
            SqlCommand selectCommand = new SqlCommand("SELECT * FROM Temp_Log_in_Contact");
            ...
            if (table.Rows.Count > 0)
            {
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    SqlCommand InsertCommand = new SqlCommand("INSERT INTO Log_in_Contact (Id, username, password, email) VALUES (@Id, @username, @password, @email)", db.getConnection);
            ...
            SqlCommand deleteCommand = new SqlCommand("DELETE FROM Temp_Log_in_Contact", db.getConnection);
