...
namespace _20142178_TranManhHuy_QLSV
{
    public partial class AuthenticationUserForm : Form
    {
        ...
        {
            fillGrid(new SqlCommand("SELECT id AS [ID], username AS [User Name], password AS [Password], email AS [Email] FROM Temp_Log_in"));
        }
        ...
        private void ButtonApprove_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(stringcon))
            {
                ...
                foreach (string userId in selectedIds)
                {
                    ...
                    {
                        // Thêm thông tin user vào bảng chính
                        SqlCommand insertCommand = new SqlCommand("INSERT INTO Log_in (Id, username, password, email) VALUES (@Id, @username, @password, @email)", connection);
            ...
            SqlCommand command = new SqlCommand("SELECT id AS [ID], username AS [User Name], password AS [Password], email AS [Email] FROM Temp_Log_in");
            dataGridViewTemp_UserList.ReadOnly = true;
            dataGridViewTemp_UserList.DataSource = user.getUsers(command);
        }
        ...
        private void ButtonApproveAll_Click(object sender, EventArgs e)
        {
            ...
            if (table.Rows.Count > 0)
            {
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    SqlCommand InsertCommand = new SqlCommand("INSERT INTO Log_in (Id, username, password, email) VALUES (@Id, @username, @password, @email)", db.getConnection);
